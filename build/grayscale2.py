...
def topath( image ) :
	image.save(rootpath + r"\.imagetemp\sketch_temp.jpg")
	return rootpath + r"\.imagetemp\sketch_temp.jpg"



net_G = build_res_unet(n_input=1, n_output=1, size=256)
net_G.load_state_dict(torch.load(rootpath + r"\res34-unet_forBlack.pt", map_location=device))
...
def sketchtograyscale(image):
	...
	matplotlib.image.imsave(rootpath + r"\.imagetemp\gray_temp.jpg", fake_imgs, cmap='gray')

	img2 = Image.open(rootpath + r"\.imagetemp\gray_temp.jpg")
